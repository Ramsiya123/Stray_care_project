...
<?php
include 'connect.php';
$res1=$con->query("SELECT COUNT(*) AS collectcount FROM collected_tb");
        if($res1){
            // $userCount=mysqli_num_rows($res1);
            $data=mysqli_fetch_assoc($res1);

        }
        $res2=$con->query("SELECT COUNT(*) AS fostercount FROM collected_tb where status='fostering'");
        if($res2){
            $data2=mysqli_fetch_assoc($res2);
        }
        $res4=$con->query("SELECT COUNT(*) AS adoptcount FROM collected_tb where status='adopted'");
        if($res4){
            $data4=mysqli_fetch_assoc($res4);
        }
        $res5=$con->query("SELECT COUNT(*) AS missingcount FROM missing_tb where status='missing'");
        if($res5){
            $data5=mysqli_fetch_assoc($res5);
        }
        $res6=$con->query("SELECT COUNT(*) AS foundcount FROM missing_tb where status='found'");
        if($res6){
            $data6=mysqli_fetch_assoc($res6);
        }
        $res7=$con->query("SELECT COUNT(*) AS deathcount FROM collected_tb where status='dead'");
        if($res7){
    ...
    <main class="app-content">
      ...
      <div class="row">
        <div class="col-md-6 col-lg-3">
          <div class="widget-small primary coloured-icon"><i class="icon fa fa-snowflake-o"></i>
            <div class="info">
              <a href="collect.php">
              <h4>Collections</h4></a>
              <p><b><?php echo $data ['collectcount']; ?></b></p>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="widget-small info coloured-icon"><i class="icon fa fa-snowflake-o"></i>
            <div class="info">
              <a href="foster.php">
              <h4>Fosterings</h4></a>
              <p><b><?php echo $data2 ['fostercount']; ?></b></p>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
            <div class="widget-small warning coloured-icon"><i class="icon fa fa-snowflake-o"></i>
              <div class="info">
                <a href="adopt.php">
                <h4>Adoptions</h4></a>
                <p><b><?php echo $data4 ['adoptcount']; ?></b></p>
              </div>
            </div>
          </div>
        ...
        <div class="col-md-6 col-lg-3">
          <div class="widget-small warning coloured-icon"><i class="icon fa fa-snowflake-o"></i>
            <div class="info">
              <a href="missing.php">
              <h4>Missings</h4></a>
              <p><b><?php echo $data5 ['missingcount']; ?></b></p>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="widget-small danger coloured-icon"><i class="  icon fa fa-snowflake-o"></i>
            <div class="info">
              <a href="found.php">
              <h4>Found</h4></a>
              <p><b><?php echo $data6 ['foundcount']; ?></b></p>
            </div>
          </div>
        </div>
        <div class="col-md-6 col-lg-3">
          <div class="widget-small primary coloured-icon"><i class="icon fa fa-snowflake-o"></i>
            <div class="info">
              <a href="dead.php">
              <h4>Dead</h4></a>
              <p><b><?php echo $data7 ['deathcount']; ?></b></p>
            </div>
          </div>
        </div>
      </div>
